...
Le pipeline CI est défini dans **Annexe 01** (`.github/workflows/ci.yml`) et s’exécute sur **GitHub Actions**. Le CD Netlify est décrit dans [1.1.5 La mise en place de la livraison ou déploiement continu (CD)](../1.1 Les bases de la démarche DevOps/1.1.5 La mise en place de la livraison ou déploiement continu (CD).md).
...
| 5 | Tests | `npm run test:run` (48 tests Vitest) |
...
- **Référence document :** « CI : workflow *CI* (lint, 48 tests, audit prod, build, E2E). CD : Netlify après required checks (`netlify.toml`, publish `build/`). Voir [1.1.5 La mise en place de la livraison ou déploiement continu (CD)](../1.1 Les bases de la démarche DevOps/1.1.5 La mise en place de la livraison ou déploiement continu (CD).md). »
...
| **Annexe 09** | [Annexe_09_DEMO_EPREUVE.md](../Annexes/Annexe_09_DEMO_EPREUVE.md) | Procédure d’exécution des tests (commandes) |
